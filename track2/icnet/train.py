...
class us3d_trainer(object):
    ...
    def load_train_data(self, num=1):
        ...
        for i in range(num):
            self.Y0[i] = to_categorical(cv2.resize(self.train_truth[i,:,:], (height, width), interpolation=cv2.INTER_NEAREST).astype(int), self.n_classes).reshape((height, width, -1))   
            self.Y1[i] = to_categorical(cv2.resize(self.train_truth[i,:,:], (height//4, width//4), interpolation=cv2.INTER_NEAREST).astype(int), self.n_classes).reshape((height//4, width//4, -1))   
            self.Y2[i] = to_categorical(cv2.resize(self.train_truth[i,:,:], (height//8, width//8), interpolation=cv2.INTER_NEAREST).astype(int), self.n_classes).reshape((height//8, width//8, -1))
            self.Y3[i] = to_categorical(cv2.resize(self.train_truth[i,:,:], (height//16, width//16), interpolation=cv2.INTER_NEAREST).astype(int), self.n_classes).reshape((height//16, width//16, -1))    
...
if __name__ == "__main__":
    ...
    # Save memory using gradient checkpointing
